...
class Register extends Core_Controller
{
        ...
        */
        ...
        if ($this->configItem('activation', 'authentication'))
        {
            ...
            if ($tokenReg === true)
            {
                ...
                $mail->SMTPAuth = true;
                $mail->Username = $this->config['EmailUsername'];
    ...
    private function process($data)
    {
        ...
        if ($accountId = $this->authentication->register($data['identity'], $data['password'], $data['email'], $recruiter))
        {
            ...
            //insert web record
            $insert = $this->db->prepare("REPLACE INTO `account_data` (`id`, `displayName`, `secretQuestion`, `secretAnswer`, `last_ip`, `reg_ip`, `last_login`, `last_login2`, `status`, `selected_realm`) VALUES (:accid, :displayName, :birthday, :country, :secretQuestion, :secretAnswer, :lastip, :regip, '0000-00-00 00:00:00', :lastlogin2, :status, :realm);");
            ...
            ######################################
            ############# LOGIN ##################
            $this->user->setLoggedIn($accountId, $this->authentication->makeHash($data['identity'], $data['password']));
