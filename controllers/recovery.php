...
class Recovery extends Core_Controller
{
    ...
    public function password_finish()
    {
        ...
        $shapasshash = $this->authentication->makeHash($row['identity'], $password);
        
        //check if the account was affected
        if ($this->authentication->changePassword($row['id'], $row['identity'], $password))
        {
            //Destroy this token
            $token->destroyToken();
            ...
            // LOGIN
            $this->user->setLoggedIn($row['id'], $shapasshash);
