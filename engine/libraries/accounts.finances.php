...
{	
		...
		if ($this->currency)
		{
			...
			if (is_numeric($amount))
			{
				...
				if (isset($this->amounts[$this->currency]))
				{
					...
					$this->amounts[$this->currency] = $this->amounts[$this->currency] + (int)$amount;
				}
				else
				{
					$this->amounts[$this->currency] = (int)$amount;
				}
	...
	*/
	public function CheckBalance()
	{
		...
		if ($CORE->user->isOnline())
		{
			...
			if ($this->account == $CORE->user->get('id'))
			{
				foreach ($this->CurrencyTranslations as $id => $key)
				{
					$balance[$id] = $CORE->user->get($key);
				}
				unset($id, $key);
			}
			else
	...
	*/
	...
	public function Charge($CA_SourceString, $CA_SourceType = CA_SOURCE_TYPE_NONE)
	{
		...
		$CORE->loadLibrary('coin.activity');
		...
		if ($update->rowCount() > 0)
		{
			...
			foreach ($this->amounts as $id => $amount)
			{
                $assoc = ($id == CURRENCY_SILVER) ? 'silver' : 'gold';

                //Update the user currency
                $CORE->user->set($assoc, (int)$CORE->user->get($assoc) - (int)$amount);
