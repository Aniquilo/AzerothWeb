...
{
	static public function AddRefundable($entry, $realmId, $price, $currency, $character, $account = false)
	{
		global $CORE, $DB;
		
		if (!$account)
			$account = $CORE->user->get('id');
		
		$time = $CORE->getTime();
		$status = IRS_STATUS_NONE;
		
		$insert = $DB->prepare("INSERT INTO `refundable_items` (`entry`, `realmId`, `price`, `currency`, `character`, `account`, `time`, `status`) VALUES (:entry, :realm, :price, :currency, :character, :account, :time, :status);");
        $insert->bindParam(':entry', $entry, PDO::PARAM_INT);
        $insert->bindParam(':realm', $realmId, PDO::PARAM_INT);
	...
	static public function GetRefundables($account = false, $realmId = false)
	{
		global $CORE, $DB;
		...
		$res = $DB->prepare("SELECT * FROM `refundable_items` WHERE `account` = :account AND `realmId` = :realm AND `time` BETWEEN :start AND :end AND `status` = :status ORDER BY `time` DESC;");
