...
class Pagination
{
	...
	public function calculate_pages($total_rows, $rows_per_page, $page_num)
	{
		...
		{
        	$first_page_btn_html = '<a href="'.$this->getURI(1).'">First</a>';
        }
		...
		{
			$prev_page_btn_html = '<a href="'.$this->getURI($page_num - 1).'">Prev</a>';
		}				
		...
		{
			$next_page_btn_html = '<a href="'.$this->getURI($page_num + 1).'">Next</a>';
		}
		...
		{
			$last_page_btn_html = '<a href="'.$this->getURI($last_page).'">Last&raquo;</a>';
		}
		...
		foreach ($surrounding as $key => $value)
		{
			...
			if ($value == $page_num)
			    $page = '<li><a href="#" class="active">'.$value.'</a></li>';
			else
			    $page = '<li><a href="'.$this->getURI($value).'">'.$value.'</a></li>';
