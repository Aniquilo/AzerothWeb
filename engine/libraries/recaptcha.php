...
/**
 * NO Captcha ReCAPTCHA a.k.a reCAPTCHA Version 2.0 library
 * 
...
class Recaptcha
{
    ...
    function __construct()
    {
        ...
        $this->core->loadConfig('recaptcha');

        if ($this->core->configItem('recaptcha_secretkey', 'recaptcha') == NULL || $this->core->configItem('recaptcha_secretkey', 'recaptcha') == "")
        {
            die ("To use reCAPTCHA you must get an API key from <a href='" . $this->signup_url . "'>" . $this->signup_url . "</a>");
        }

        if ($this->core->configItem('recaptcha_sitekey', 'recaptcha') == NULL || $this->core->configItem('recaptcha_sitekey', 'recaptcha') == "")
        {
            die ("To use reCAPTCHA you must get an API key from <a href='" . $this->signup_url . "'>" . $this->signup_url . "</a>");
        }

        $this->_secret = $this->core->configItem('recaptcha_secretkey', 'recaptcha');
        $this->_sitekey = $this->core->configItem('recaptcha_sitekey', 'recaptcha');

        if ($this->core->configItem('lang', 'recaptcha') == NULL || $this->core->configItem('lang', 'recaptcha') == "")
        {
            $this->_lang = 'en';
        }
        else
        {
            $this->_lang = $this->core->configItem('lang', 'recaptcha');
        }
    }
     ...
     * @param string $remoteIp Remote IP you wish to send to reCAPTCHA, if NULL $this->core->security->getip() will be called
    ...
    public function verifyResponse($response, $remoteIp = NULL)
    {
        ...
        $getResponse = $this->_submitHttpGet(
            $this->_siteVerifyUrl, array(
                'secret' => $this->_secret,
                'remoteip' => (!is_null($remoteIp)) ? $remoteIp : $this->core->security->getip(),
                'v' => $this->_version,
                'response' => $response
            )
        );
