...
{	
	...
	static public function usernameExists($name)
	{
	    ...
	    return ($CORE->authentication->getAccountByUsername($name) !== false);
	}
	...
	static public function emailExist($email)
	{
	    ...
	    return ($CORE->authentication->getAccountByEmail($email) !== false);
	}
	...
	static public function displayNameExist($string)
	{
		...
		$res = $CORE->db->prepare("SELECT id, displayName FROM `account_data` WHERE `displayName` = :str LIMIT 1");
	...
	static public function checkUsername($username)
	{
		...
		else if (strlen($username) > 32)
		{
			...
			return 'The account name is too long, maximum length 32.';
		}
		else if (strlen($username) < 4)
		{
			//if the username is too short
			return 'The account name is too short, minimum length 4.';
		}
	...
	static public function checkDisplayname($displayName)
	{
		...
		else if (strlen($displayName) < 4)
		{
			//display name too short
			return 'The display name is too short, minimum length 4.';
		}
