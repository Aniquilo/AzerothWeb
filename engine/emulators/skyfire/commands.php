...
class skyfire_Commands implements emulator_Commands
{
    private $core;
    private $realmId;
	...
	{
		$soapMsg = $this->ExecuteCommand('.itemremove quantity '.$charName.' '.$entry);
	...
	public function TicketAppend($id, $text)
    {
        $soapMsg = $this->ExecuteCommand('.ticket response appendln '.$id.' '.$text);
    ...
    {
        $soapMsg = $this->ExecuteCommand('.ticket complete '.$id);
	...
	public function Migrate($filename, $acc, $charName)
    {
        $soapMsg = $this->ExecuteCommand('.pdump load '.$filename.' '.$acc.' '.$charName);
