...
<section id="content">
    ...
    <div class="tab" id="maintab">
        ...
        <div class="form">
            ...
            <form method="post" action="<?php echo base_url(); ?>/admin/news/submit_edit?id=<?php echo $row['id']; ?>" id="newsForm" name="editNewsForm">
                ...
                <section>
                  ...
                  <div>
                    <input id="label" name="title" type="text" class="required" value="<?php echo stripslashes($row['title']); ?>"/>
                  </div>
                </section>
                ...
                <section>
                  ...
                  <div>
                    <textarea class="required" id="textarea" rows="7" name="shortText"><?php echo stripslashes($row['shortText']); ?></textarea>
                  </div>
                </section>
                ...
                <section>
                  ...
                  <div>
                    <textarea class="tinymce" id="textarea" name="text"><?php echo $row['text']; ?></textarea>
                  </div>
                </section>

                <input type="hidden" name="image" id="newsImage" value="<?php echo $row['image']; ?>" />
                
                <?php if ($row['image'] != NULL) { ?>
                <script>
                    $(document).ready(function()
                    {
                        updatePreview('<?php echo base_url() . '/uploads/news/thumbs/' . $row['image']; ?>', '<?php echo $row['image']; ?>');
                    });
                </script>
                <?php } ?>
                ...
                <div>
                    ...
                    <form id="croppingForm" method="POST" onsubmit="return false;" name="cropForm">
                        ...
                        <div id="image_CroppingSection" style="display: none; margin-bottom: 5px;">
                            <div id="uploadedImagePreview" style="max-width: 229px; display: inline-block;"></div>
        ...
        <p>
        	<span class="button-group">
                <input type="button" class="button primary submit" value="Submit" />
                <a href="<?=base_url()?>/admin/news" class="button">Cancel</a>
         	</span>
        </p>
...
<script type="text/javascript">
	...
	{
		<?php
		if ($formData = $ERRORS->GetFormData('editNews'))
		{	
			echo '
			var savedFormData = $.parseJSON(', json_encode(json_encode($formData)), ');
			restoreFormData(\'editNewsForm\', savedFormData);';
		}
		unset($formData);
		?>
	...
	function updatePreview(imageSrc, imageName)
	{
		...
		function proceed()
		{
			...
			else
			{
				//empty the container
				previewSection.html('');
				previewSection.append('<img src="' + imageSrc + '" />');
