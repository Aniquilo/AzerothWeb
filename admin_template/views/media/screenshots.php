...
$ERRORS->PrintAny('deleteScreenshot');
?>

<script src="<?=base_url()?>/admin_template/js/shadowbox.js" type="text/javascript"></script>
<script>
...
<nav id="secondary" class="disable-tabbing">
	<ul>
		...
		<li class="current"><a href="<?=base_url()?>/admin/media/screenshots">Screenshots</a></li>
	</ul>
</nav>
                ...
                if ($res->rowCount() > 0)
                {
                    while ($arr = $res->fetch())
                    {
                        ...
                        <tr id="screen-record-', $arr['id'], '">
                            <td>', $arr['name'], '</td>
                            <td><a class="sliding-image" href="', base_url(), '/uploads/media/screenshots/', $arr['image'],'" rel="shadowbox" title="', $arr['name'], '{|}', $arr['descr'], '" style="display: block; height: 30px; overflow-y: hidden;"><img src="', base_url(), '/uploads/media/screenshots/thumbs/', $arr['image'],'" height="100" /></a></td>
                            <td>', $arr['added'], '</td>
                            <td>', $arr['account'], '</td>
                            ...
                            <span class="button-group">
                                ...
                                <a href="', base_url(), '/admin/media/screenshot_delete?id=', $arr['id'], '" onclick="return deletecheck(\'Are you sure you want to delete this screenshot?\');" class="button icon remove danger">Remove</a>
                            </span>
                            </td>
                        </tr>';
                    }
...
<script>
	...
	function ApproveScreenshot(id) {
		...
		$.post($configURL + "/admin/media/screenshot_approve", { 
            id: $id,
        }, 
        function(data) {
            //check for errors
	...
	function DenyScreenshot(id)
	{
		...
		$.post($configURL + "/admin/media/screenshot_deny", { 
            id: $id,
        }, 
        function(data) {
            //check for errors
