...
(function($)
{
	...
	var methods =
	{
		...
		init : function(options)
		{
			...
			$('#armorset-purchase-form').on('submit', function() {
				...
				{
					//no characters, fail
					$.fn.WarcryAlertBox('open', '<p>Unable to proceed, please select an armor set.</p>');
					return false;
				}
		...
		applyFilter : function(filter)
		{
			...
			WarcryQueue('ARMORSETS').add(function()
			{
				...
				//get the page data	
				$.get($BaseURL + "/armorsets/get_filter",
                {
		...
		loadPage : function(page)
		{
			...
			{
				//get the page data	
				$.get($BaseURL + "/armorsets/get_page",
					{
						page: $page,
						perPage: $config.perPage,
					...
					function(data)
					{
						...
						{	
							//loop the items
							$.each(data.armorSets, function (i, v) {
								var armorsetId = v.id;
								var order = v.order;
								var html = v.html;
								...
								WarcryQueue('ARMORSETS').add(function()
								{
									$('#armorsets-container').WarcryArmorsets('loadItems', { armorset: armorsetId, order: order, realm: $config.realm, pageItems: count });
								});
							});
						}
			...
			for (i = 0; i < items.length; i++)
			{
				...
				WarcryQueue('ARMORSETS_'+$armorsetId).add(function()
				{
                    ...
                    function(data)
                    {
                        ...
                        $('#armor-set-'+$armorsetId+'-items').append('<a class="'+quality.toLowerCase()+'" href="'+$WOWDBURL+'/?item='+$entry+'" rel="item='+$entry+'" data-realm="'+$realm+'" style="background-image:url(\'http://wow.zamimg.com/images/wow/icons/medium/'+icon.toLowerCase()+'.jpg\');" onclick="return false;"></a>');
