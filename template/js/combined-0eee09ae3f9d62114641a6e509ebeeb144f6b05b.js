...
function updateInfoPane(val)
{
	...
	// Update the bonus pane
	if ($BonusTable != null)
	{
		var bonus = false;
		
		$.each($BonusTable, function(i, v)
		{
			var range = v.range.split("-");
			
			// check if we found a suitable bonus range
			if (val >= range[0] && val <= range[1])
			{
				$('.coins-bonus-pane #bonus_percent_amount').html(v.bonus);
				$('.coins-bonus-pane #bonus_coins_amount').html(Math.floor((v.bonus / 100) * val));
				$('.coins-bonus-pane').fadeIn('fast');
				bonus = true;
